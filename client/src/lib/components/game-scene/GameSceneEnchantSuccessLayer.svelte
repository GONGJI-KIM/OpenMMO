<script lang="ts">
  ...
  useTask(() => {
    ...
    for (const event of takeEnchantSuccesses()) {
      ...
      if (!local && !remotePlayerManager.players.has(event.playerId)) continue
      let slot = local
        ? slots[0]
        : slots.find(
    ...
    for (const slot of slots) {
      ...
      const pose = local
        ? player
        : remotePlayerManager.players.get(event.playerId)
      if (elapsed >= 0.6 || !pose || (local && player.health <= 0)) {
        slot.event = null
        slot.system.clear()
        continue
      }
      anchor.set(pose.position.x, pose.position.y + 1.1, pose.position.z)
      anchor.x = unwrapWorldXNear(player.position.x, anchor.x)
      if (event.weapon) {
        if (local && $localPlayerRightHand)
          $localPlayerRightHand.getWorldPosition(anchor)
        else {
          anchor.x += Math.cos(pose.rotation) * 0.35
          anchor.z -= Math.sin(pose.rotation) * 0.35
        }
      }
      if (!local && !frustum.containsPoint(anchor)) {
        slot.system.clear()
        continue
      }
      slot.system.update(
